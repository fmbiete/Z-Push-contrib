create table settings (key_name varchar(50) not null, value varchar(50) not null, created_at datetime not null, updated_at datetime not null, primary key (key_name));
...
create table states (id_state integer auto_increment, devid varchar(50) not null, key_value varchar(50), type varchar(50), counter integer, data text not null,
            created_at datetime not null, updated_at datetime not null, primary key (id_state));

create unique index idx_states_unique on states (devid, key_value, type, counter);
